...
(define wayland-shared-files
  "/share/wayland-protocols")

(define var
  "WAYLAND_PROTOCOLS_XMLS_PATH")
  ...
  (package
   ...
   (arguments
    (list
     ...
     #~(modify-phases
        %standard-phases
        (add-before 'configure 'export-xml-paths
                    (lambda* (#:key inputs outputs #:allow-other-keys)
                      (map (lambda (i)
                             (let [(path (string-append (cdr i) #$wayland-shared-files))]
